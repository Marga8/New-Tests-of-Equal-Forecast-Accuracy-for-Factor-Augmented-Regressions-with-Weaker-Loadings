...
power_values_alpha1_051 <- c(0.056,
                             0.14,
                             0.582,
                             0.786,
                             0.828,
...
power_values_alpha1_01 <- c(0.052,
                            0.128,
                            0.498,
                            0.714,
                            0.766,
...
p <- ggplot(data) +
  ...
  ggtitle(expression("Power Evolution for Different " * alpha[i] * " Values, " * lambda[1]^0 * " = 1, " * lambda[2]^0 * " = 0.65")) +  # Add a title with Greek letters
  xlab(expression(" " * beta * " Values")) +                                    # Label the x-axis
  ylab(expression("Power  " * g[2*","*f,]^{adj})) +                             # Label the y-axis
...
p + annotate("text", x = 0.525, y = 0.05, label = "size = 0.052", color = "orange", hjust = 0, size = 5) +  # for alpha = 0.1
  annotate("text", x = 0.525, y = 0.1, label = "size = 0.056", color = "green", hjust = 0, size = 5) +   # for alpha = 0.51
